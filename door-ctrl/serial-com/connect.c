...
#define ORG_Y 0
#define ORG_Z 0
#define ALPHA (1.0 / 2.0 * M_PI) - 1.0 / 9.0 * M_PI //取り付け角
//#define ALPHA 74.0 / 180.0 * M_PI //取り付け角
...
int main ()
{
    ...
    //-角度用-//
    long U_x_beta[20][144];
    long U_y_beta[20][144];
    long D_x_beta[20][144];
    long D_y_beta[20][144];
    double U_angle_x[20][144];
    double U_angle_y[20][144];
    double D_angle_x[20][144];
    double D_angle_y[20][144];
    ...
    while(1){ //距離データ取得ループ
        ...
        if(det == 'U')
        {
            ...
            for(i=0; i<20; i++)
            {
                for(j=0; j<144; j++)
                {
                    calc_xyz(U_angle_x[i][j], U_angle_y[i][j], dist[i][j], &SCAN_DATA.x[i*144 + j], &SCAN_DATA.y[i*144 + j], &SCAN_DATA.z[i * 144 + j]);
                }
            }
        }else if(det == 'D'){
            SCAN_DATA.det = 'D';
            for(i=0; i<20; i++)
            {
                for(j=0; j<144; j++)
                {
                    calc_xyz(D_angle_x[i][j], D_angle_y[i][j], dist[i][j], &SCAN_DATA.x[i*144 + j], &SCAN_DATA.y[i*144 + j], &SCAN_DATA.z[i * 144 + j]);
                }
            }
        }
        ...
        for(i=0; i<20; i++)
        {
            for(j=0; j<144; j++)
            {
                SCAN_DATA.dist[144*i + j] = dist[i][j];
            }
        }
